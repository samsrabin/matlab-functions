...
%     end
    ...
    % Sam Rabin (SSR) 2018-04-14
    linec_ssr = linec ;
    if any(isnan(x)) || any(isnan(y))
%         warning(['NaN(s) in x (' num2str(length(find(isnan(x)))) '/' num2str(length(x)) ') and/or y (' num2str(length(find(isnan(y)))) '/' num2str(length(y)) ').'])
        if all(isnan(x)) && all(isnan(y))
            warning(['Patch ' num2str(k) ': All ' num2str(length(x)) ' vertices (NaN,NaN)! Skipping.'])
            continue
        elseif isequal(isnan(x),isnan(y))
            if length(find(~isnan(x)))<3
                warning(['Patch ' num2str(k) ': Only ' length(find(~isnan(x))) ' vertices not (NaN,NaN)! Skipping.'])
                continue
            else
                warning(['Patch ' num2str(k) ': Only ' num2str(length(find(~isnan(x)))) '/' num2str(length(x)) ' vertices not (NaN,NaN). Coloring red.'])
                x = x(~isnan(x)) ;
                y = y(~isnan(y)) ;
                linec_ssr = 'red' ;
            end
        else
            error('How do NaN values not match up in vertex coordinates??')
        end
    end
    ...
    if strcmp(STYL,'speckle') || strcmp(STYL,'outspeckle')
        if any(xhatch)
            H(j) = line(xhatch,yhatch,'marker','.','linest','none', ...
                    'markersize',specksize,'color',linec_ssr);
            j = j+1;
        end
    elseif strcmp(STYL,'single') || strcmp(STYL,'cross')
        H(j) = line(xhatch,yhatch);
        set(H(j),'color',linec_ssr,'linewidth',linew);
        j = j+1;
    end
end
